...
export default function createFocusStore({
  ...
  function createNodes(nodeHierarchy: Node[], nodeDefinitionHierarchy: NodeDefinition[]) {
    ...
    const isUnlockingFocus = !shouldLockFocus && currentState._updatingFocusIsLocked;
    ...
    if (hasNodesToUpdate || isUnlockingFocus) {
      ...
      if (shouldLockFocus) {
        ...
        const focusedItemIndex = nodeHierarchyIds.indexOf(currentState.focusedNodeId);
  ...
  function updateNode(nodeId: Id, update: NodeUpdate) {
    ...
    if (update && nodeChanged) {
      const newNode: Node = {
        ...
        defaultFocusColumn: update.defaultFocusColumn ?? currentNode.defaultFocusColumn,
        ...
        forgetTrapFocusHierarchy:
          update.forgetTrapFocusHierarchy ?? currentNode.forgetTrapFocusHierarchy,
        defaultFocusChild: update.defaultFocusChild ?? currentNode.defaultFocusChild,
      };
      ...
      const nodeWasFocused = currentState.focusHierarchy.find((v) => v === nodeId);
